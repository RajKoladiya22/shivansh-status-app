...
export default function GeneralUpdatePage() {
  ...
  */
    ...
    setWorkedOn(
      workedOn.map((w, idx) =>
        idx === projIdx
          ? {
              ...w,
              tasks: w.tasks.map((t, tIdx) => (tIdx === taskIdx ? { ...t, ...patch } : t)),
            }
          : w,
      ),
    );
  ...
  const updateInProgressProject = (
    ...
    setInProgress(inProgress.map((p, idx) => (idx === i ? { ...p, ...patch } : p)));

  const addInProgressTask = (projIdx: number) =>
    setInProgress(
      inProgress.map((p, idx) => (idx === projIdx ? { ...p, tasks: [...p.tasks, ""] } : p)),
    );

  const removeInProgressTask = (projIdx: number, taskIdx: number) =>
    setInProgress(
      inProgress.map((p, idx) =>
        idx === projIdx ? { ...p, tasks: p.tasks.filter((_, i) => i !== taskIdx) } : p,
      ),
    );
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white shadow-md rounded-xl p-6 space-y-6">
            ...
            {/* Worked On */}
            <div>
              <label className="text-sm font-semibold text-gray-800">Worked-On:</label>
              <div className="space-y-3 mt-2">
                {workedOn.map((w, i) => (
                  <div key={i} className="p-3 border border-gray-400 rounded-lg bg-gray-50 space-y-3">
                    <div className="flex gap-3 items-center">
                      <input
                        value={w.project}
                        onChange={(e) => updateWorkedProject(i, { project: e.target.value })}
                        placeholder="Project Name"
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                      />

                      {workedOn.length > 1 && (
                        <button onClick={() => removeWorkedProject(i)} className="text-red-600 border border-red-300 rounded p-1">
                          <X className="w-4 h-4" />
                        </button>
                      )}

                      {i === workedOn.length - 1 && (
                        <button onClick={addWorkedProject} className="text-green-600 border border-green-300 rounded p-1">
                          <Plus className="w-4 h-4" />
                        </button>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      {w.tasks.map((t, ti) => (
                        <div key={ti} className="flex flex-wrap gap-3 items-center">
                          <input
                            value={t.desc}
                            onChange={(e) => updateWorkedTask(i, ti, { desc: e.target.value })}
                            placeholder="Task"
                            className="flex-1 px-3 py-2 border border-gray-300 rounded-lg"
                          />
                          ...
                          <input
                            type="number"
                            value={t.hrs}
                            onChange={(e) => updateWorkedTask(i, ti, { hrs: e.target.value })}
                            placeholder="hrs"
                            className="w-24 px-3 py-2 border border-gray-300 rounded-lg"
                          />

                          {w.tasks.length > 1 && (
                            <button onClick={() => removeWorkedTask(i, ti)} className="text-red-600 border border-red-300 rounded p-1">
                              <X className="w-4 h-4" />
                            </button>
                          )}

                          {ti === w.tasks.length - 1 && (
                            <button onClick={() => addWorkedTask(i)} className="text-green-600 border border-green-300 rounded p-1">
                              <Plus className="w-4 h-4" />
                            </button>
                          )}
                        </div>
            ...
            {/* In Progress */}
            <div>
              <label className="text-sm font-semibold text-gray-800">In-Progress:</label>
                ...
                {inProgress.map((p, i) => (
                  <div key={i} className="p-3 border border-gray-400 rounded-lg bg-gray-50 space-y-3">
                    <div className="flex gap-3 items-center">
                      <input
                        value={p.project}
                        onChange={(e) => updateInProgressProject(i, { project: e.target.value })}
                        placeholder="Project Name"
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                      />

                      {inProgress.length > 1 && (
                        <button onClick={() => removeInProgressProject(i)} className="text-red-600 border border-red-300 rounded p-1">
                          <X className="w-4 h-4" />
                        </button>
                      )}

                      {i === inProgress.length - 1 && (
                        <button onClick={addInProgressProject} className="text-green-600 border border-green-300 rounded p-1">
                          <Plus className="w-4 h-4" />
                        </button>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      {p.tasks.map((t, ti) => (
                        <div key={ti} className="flex gap-3 items-center">
                          <input
                            value={t}
                            onChange={(e) => updateInProgressTask(i, ti, e.target.value)}
                            placeholder="Task"
                            className="flex-1 px-3 py-2 border border-gray-300 rounded-lg"
                          />

                          {p.tasks.length > 1 && (
                            <button onClick={() => removeInProgressTaskClean(i, ti)} className="text-red-600 border border-red-300 rounded p-1">
                              <X className="w-4 h-4" />
                            </button>
                          )}

                          {ti === p.tasks.length - 1 && (
                            <button onClick={() => addInProgressTask(i)} className="text-green-600 border border-green-300 rounded p-1">
                              <Plus className="w-4 h-4" />
                            </button>
                          )}
                        </div>
            ...
            {/* Query */}
            <div>
              <label className="text-sm font-semibold text-gray-800">Query:</label>

              <div className="space-y-3 mt-2">
                {queries.map((q, i) => (
                  <div key={i} className="flex flex-wrap gap-3 p-3 border border-gray-400 rounded-lg bg-gray-50">
                    <input
                      value={q.project}
                      onChange={(e) => updateQuery(i, { project: e.target.value })}
                      placeholder="Project Name"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                    />
                    ...
                    {i === queries.length - 1 && (
                      <button onClick={addQuery} className="text-green-600 border border-green-300 rounded p-1">
                        <Plus className="w-4 h-4" />
                      </button>
                    )}

                    {queries.length > 1 && (
                      <button onClick={() => removeQuery(i)} className="text-red-600 border border-red-300 rounded p-1">
                        <X className="w-4 h-4" />
                      </button>
                    )}
                  </div>
          ...
          <div className="bg-white shadow-md rounded-xl p-6 h-fit relative">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={copyMessage} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700">
                {copied ? (
                  <>
                    <Check className="w-4 h-4" /> Copied!
                  </>
            ...
            <div className="border border-gray-400 p-4 rounded-lg bg-gray-50 min-h-[300px] mb-6">
              <pre className="whitespace-pre-wrap text-sm ">{buildMessage()}</pre>
